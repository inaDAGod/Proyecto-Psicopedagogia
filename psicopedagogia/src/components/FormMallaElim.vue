<template>
    <div class="modal" v-show="showForm">
      <div class="modal-content">
        ...
        <h2>Eliminar Malla</h2>
        <p style="background-color: white;padding: 2%;border-radius: 20px;font-size: 140%;">¿Estás seguro de que deseas eliminar <br> {{semestre.sigla }}?</p>
  ...
  const submitForm = async () => {
    try {
      const response = await axios.delete(`http://localhost:3000/api/semestre/${props.semestre._id}`);
      if (response.status === 200) {
        console.log('semestre eliminado correctamente');
        closeForm();
      } else {
        console.error('Error al eliminar el semestre:', response.statusText);
      }
    } catch (error) {
